...
export default function TicketCard({
  ...
  return (
    ...
    >
      ...
      {!isCompleted && (
        <div
          className={`px-4 py-2 border-b border-purple/10 ${
            esc ? "bg-blue-900/10" : "bg-surface2"
          }`}
        >
          {esc ? (
      ...
      <div className="px-4 py-3 space-y-2 text-sm">
        ...
        {!isCompleted && (
          <div className="flex items-center gap-2 pt-1">
            ...
            <select
              onChange={(e) =>
                onStatusChange(t.issue_key, e.target.value)
              }
      ...
      <div className="px-4 pt-3 pb-2 border-t border-purple/10 flex items-center gap-2">
        <button
          onClick={() =>
            window.open(`/runbooks?id=${t.issue_key}`, "_blank")
          }
        ...
        <button
          ...
          className="flex-1 bg-red/10 hover:bg-red/20 border border-red/20 text-red text-[0.65rem] font-bold py-2 px-3 rounded-xl font-mono transition-all duration-200 hover:scale-[1.02]"
        >
          🔍 RCA
        </button>
